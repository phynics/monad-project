...
- [ ] Task: Implement Feature: Refine `ContextManager` to inject all notes globally and use vector-based recall for memories only.
- [ ] Task: Write Tests: Verify that the SQL execution proxy correctly identifies "sensitive" operations and requests user confirmation.
- [ ] Task: Implement Feature: Add a confirmation proxy in `ChatViewModel` that intercepts `ExecuteSQLTool` calls and triggers a SwiftUI confirmation dialog for specific patterns (e.g., `CREATE TABLE`, `DROP TABLE`).
- [ ] Task: Conductor - User Manual Verification 'Context Engine Refinement and UI Proxy' (Protocol in workflow.md)

...
public struct ReadFileTool: Tool, @unchecked Sendable {
    public let id = "cat"
    ...
    public func execute(parameters: [String: Any]) async throws -> ToolResult {
        ...
        do {
            ...
            if size > 1_000_000 { // 1MB limit for raw cat
                return .failure("File is too large (\(ByteCountFormatter.string(fromByteCount: size, countStyle: .file))). Please use document tools to load it as context.")
            }

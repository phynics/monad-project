...
public final class CreateMemoryTool: Tool, Sendable {
    ...
    private let logger = Logger(
        subsystem: Bundle.main.bundleIdentifier ?? "com.monad.shared", category: "CreateMemoryTool")
        ...
        return [
            ...
            "properties": [
                ...
                "tags": [
                    ...
                    "items": ["type": "string"],
                    "description":
                        "Optional tags for categorization. If not provided, keywords will be extracted automatically.",
                ],
            ],
    ...
    public func execute(parameters: [String: Any]) async throws -> ToolResult {
        ...
        do {
            ...
            let savedId = try await persistenceService.saveMemory(
                memory, policy: .preventSimilar(threshold: 0.92))

            if savedId == memory.id {
                logger.info("Successfully created memory: \(title)")
                return .success(
                    "Memory '\(title)' created successfully with semantic embedding and \(tags.count) tags."
                )
            } else {
                logger.info("Skipped creation of duplicate memory: \(title)")
                return .success(
                    "Memory creation skipped: A semantically similar memory already exists.")
            }
        } catch {

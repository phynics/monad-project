...
public struct FindExcerptsTool: Tool, @unchecked Sendable {
    public let id = "find_excerpts"
    public let name = "Find Excerpts"
    public let description = "Launch a specialized subagent to scan a document and find specific information. It returns a list of recommended offsets and lengths that you can use with `switch_document_view` in `excerpt` mode."
    ...
    public var parametersSchema: [String: Any] {
        return [
            ...
            "properties": [
                ...
                "search_instruction": [
                    "type": "string",
                    "description": "Describe what specific information or code section you are looking for"
                ]
            ],
    ...
    public func execute(parameters: [String: Any]) async throws -> ToolResult {
        guard let path = parameters["path"] as? String,
              let instruction = parameters["search_instruction"] as? String else {
        ...
        let (stream, _, _) = await llmService.chatStreamWithContext(
            ...
            chatHistory: [],
            tools: [],
            systemInstructions: "You are a document scanner subagent. Your goal is to find specific information in a document and report its exact character offset and length. Be precise.",
            useFastModel: true
        )
        ...
        let context = SubagentContext(prompt: subagentPrompt, documents: [path], rawResponse: response)
        return .success("Subagent Findings for '\(instruction)':\n\n\(response)\n\nUse `switch_document_view` with mode `excerpt` and the provided offset/length to read these sections.", subagentContext: context)
    }
}

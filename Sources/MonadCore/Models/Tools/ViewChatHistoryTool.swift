...
public struct ViewChatHistoryTool: Tool, Sendable {
    public let id = "view_chat_history"
    public let name = "View Chat History"
    public let description =
        "Fetch past messages from the current or a specific conversation session. Use this if the prompt history has been truncated."
    ...
    public init(
        ...
        currentSessionProvider: @escaping @Sendable () async -> UUID?
    ) {
    ...
    public func execute(parameters: [String: Any]) async throws -> ToolResult {
        ...
        do {
            ...
            return .success(
                "History (Total: \(messages.count), Showing: \(page.count), Offset: \(offset)):\n\n\(formatted)"
            )

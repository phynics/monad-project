...
public struct LoadArchivedChatTool: Tool, @unchecked Sendable {
    public let id = "load_archived_chat"
    public let name = "Load Archived Chat"
    public let description = "Load an archived conversation transcript into the active context as a document"
    ...
    public func execute(parameters: [String: Any]) async throws -> ToolResult {
        guard let idString = parameters["session_id"] as? String, let sessionId = UUID(uuidString: idString) else {
        ...
        do {
            ...
            return .success("Archived conversation '\(session.title)' loaded as document '\(path)'. You can now use document tools to read it.")

...
public actor PersistenceService {
    ...
    public func saveMemory(_ memory: Memory, policy: MemorySavePolicy = .always) throws -> UUID {
        // If check requested and we have an embedding, verify uniqueness
        if case .preventSimilar(let threshold) = policy {
            let vector = memory.embeddingVector
            if !vector.isEmpty {
                // Search for very similar memories (high threshold)
    ...
    public func vacuumMemories(threshold: Double = 0.95) throws -> Int {
        ...
        let allMemories = try fetchAllMemories() // Sorted by updatedAt desc
        var deletedCount = 0
        
        // Naive O(N^2) comparison - fine for expected scale (< 1000 active memories)
        // We process from newest to oldest (because fetchAllMemories returns desc).

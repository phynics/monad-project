...
public final class PersistenceManager {
    ...
    private var debugInfoCache: [UUID: MessageDebugInfo] = [:]
    ...
    /// Replace all messages in the current session (used for context compression/summarization)
    public func replaceMessages(with newMessages: [Message]) async throws {
        ...
        // This is a heavy operation but ensures consistency for now
        // A better approach would be diffing, but that's complex with the forest structure
        ...
        for msg in newMessages {
            let dbMsg = ConversationMessage(
                ...
                recalledMemories: "[]", // We lose memory links on re-insert if we don't map them back. 
                                      // Ideally Message should preserve them.
                                      // For summaries, they are new. For existing, we should preserve.
                                      // But Message model has recalledMemories: [Memory]?.
                memoryId: nil,
            ...
            // Re-encoding recalledMemories
            if let memories = msg.recalledMemories,
               let data = try? JSONEncoder().encode(memories),
               let str = String(data: data, encoding: .utf8) {
                var mutMsg = dbMsg
                mutMsg.recalledMemories = str
                try await persistence.saveMessage(mutMsg)
            } else {
    ...
    public func saveMemory(_ memory: Memory, policy: MemorySavePolicy = .always) async throws -> UUID {
        ...
        return try await persistence.saveMemory(memory, policy: policy)
    }
    ...
    public func vacuumMemories() async throws -> Int {
        logger.info("Triggering manual memory vacuum")
        return try await persistence.vacuumMemories()
    }

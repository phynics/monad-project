...
@Suite(.serialized)
@MainActor
struct ConversationArchiverTests {
    ...
    init() async throws {
        ...
        archiver = ConversationArchiver(persistence: persistence, llmService: mockLLM, embeddingService: mockEmbeddingService)
    }
    ...
    func archive_generatesTitleFromFirstUserMessage() async throws {
        ...
        // When
        let sessionId = try await archiver.archive(messages: messages, sessionId: nil)
    ...
    func archive_indexesLongMessagesAsMemories() async throws {
        ...
        // When
        _ = try await archiver.archive(messages: messages, sessionId: nil)
    ...
    func archive_associatesMessagesWithSession() async throws {
        ...
        // Then
        let storedMessages = try await persistence.fetchMessages(for: sessionId)
        #expect(storedMessages.count == 2)
        #expect(storedMessages[0].content == "Message 1")
        #expect(storedMessages[1].content == "Message 2")
        #expect(storedMessages[0].sessionId == sessionId)
        #expect(storedMessages[1].sessionId == sessionId)
    }

    @Test
    func archive_updatesExistingSession() async throws {
        // Given
        var existingSession = Timeline(title: "Old Title")
        try await persistence.saveSession(existingSession)
        ...
        // When
        let sessionId = try await archiver.archive(messages: messages, sessionId: existingSession.id)

        // Then
        #expect(sessionId == existingSession.id)
        let updatedSession = try await persistence.fetchSession(id: existingSession.id)
    ...
    func archive_handlesEmptyMessages() async throws {
        // When
        let sessionId = try await archiver.archive(messages: [], sessionId: nil)
